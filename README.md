...
В рамках третьего задания были добавлены unit-тесты и Makefile для автоматизации основных задач разработки.

### Unit-тесты

Создана директория `tests/` с двумя unit-тестами:

- `tests/test_models.py`  
  Обычный unit-тест без внешних зависимостей. Проверяет базовую логику модуля `models.py`.
  Тест изолирован: внешние вызовы (S3, DVC, MLflow) замоканы с помощью `mocker.patch`.

- `tests/test_storage.py`  
  Тест с мокингом S3-хранилища. Проверяет полный цикл работы с моделями в MinIO-подобном хранилище.
...
В файле IMAGE_NAME = yourusername/mlops-task1 - yourusername заменить на свой логин в Docker Hub
Доступные команды:
